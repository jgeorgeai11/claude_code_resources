"""Logging configuration for entry-point scripts and library modules.
...
`get_logger()` for every module. A project that needs logging in CI
vendors a copy into its own tree rather than importing from untracked
`.claude/` content; such copies fork from this one by design (updates
here do not propagate to them).
"""
